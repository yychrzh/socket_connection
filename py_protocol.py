...
class Data_transfer(Number_conver, Tcpsocket):
    ...
    def float2send_byte(self, float_data, bit=32):
    ...
    def send_data(self, float_array, bit=32):
        send_bys = bytes(self.float2send_byte(float_array, bit))
        self.debug_print("send_bys", send_bys, len(send_bys))
        self.send_bytes(send_bys)

    def recv_data(self):
        float_array = []
        recv_char = self.recv_bytes()
        recv_bys = self.recv_char2byte(recv_char)
